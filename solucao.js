...
function solucao1(somaEstoque){

    somaEstoque = listaProdutos.reduce((totalSoma,item) => {
        return totalSoma + item.qtdEstoque
    }, 0)
    return console.log(`Quantidade total de itens no estoque: ${somaEstoque}`)
}
...
function solucao2(somaDestaque, totalSim = 0){
    
    somaDestaque = listaProdutos.forEach((item) => {
        if (item.emDestaque === "sim"){
            totalSim++
        }
    });

    return console.log(`Total de itens em destaque: ${totalSim}`)
}
...
function solucao3(totalDisponivel, totalD = 0){

    totalDisponivel = listaProdutos.forEach((item) => {
        if (item.disponivel === "sim"){
            totalD++
        }
    });
    return console.log (`Total de itens disponíveis: ${totalD}`)
}
...
function solucao4(totalDisponiveleDestaque, totalDE = 0){

    totalDisponiveleDestaque = listaProdutos.forEach((item) => {

        if ((item.disponivel === 'sim') && (item.emDestaque === 'sim')){
            totalDE++
        }
    });
    return console.log(`Total de itens disponíveis e em destaque: ${totalDE}`);
}
...
function solucao5( ){
    ...
    estoque+= item.preco * item.qtdEstoque
    });

    return console.log(`Valor total  do inventário: ${estoque.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`);
}
...
function solucao6(){
    listaProdutos.forEach((item) =>{
        if (item.preco > valorMaior){
            valorMaior = item.preco;
            lista = item;
        }
    })
    return console.log(`O produto mais caro é ${lista.descricao}, do departamento de ${lista.departamento.nomeDepto}, no valor de ${valorMaior.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`);
...
/* 7. Produto mais barato da loja (bem como seu departamento - considere apenas o preço dele) */
 ...
 function solucao7(valorMenor){
    valorMenor = valorMaior
    listaProdutos.forEach((item) => {
        if( item.preco < valorMenor){
            valorMenor = item.preco
            lista = item
        }
    })

    return console.log((`O produto mais barato é ${lista.descricao}, do departamento de ${lista.departamento.nomeDepto}, no valor de ${valorMenor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}`))
}
...
function solucao9(menosValioso){
